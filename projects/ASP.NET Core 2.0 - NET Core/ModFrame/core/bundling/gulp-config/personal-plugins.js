...
//Variables used for the plugin directory paths etc
var compiledDirectory = path.join(hosting, 'components', 'extra-plugins', 'p');
var originalDirectory = './plugins/personal';
var folderName = "personal-plugins";
...
gulp.task('MF-personal-plugins-watch', function ()
{
    ...
    chokidar.watch([watchJSPath, watchCSSPath, watchLessPath, watchSassPath], { ignoreInitial: true, depth: 5 })
        .on('all', function (event, path) {
            ...
            gulp.start(["MF-personal-plugins-clean"]);

            //Triggers the update method
            if (env.isDevelopment()) {
                gulp.start(['MF-debug-personal-plugins']);
            }
            else {
                gulp.start(['MF-personal-plugins']);
            }
        })
        .on('error', function () {
...
gulp.task('MF-personal-plugins-clean', function ()
{
...
gulp.task('MF-personal-plugins', function ()
{
...
//Compiles all the personal plugin js and css in minified format split up into their corraspending plugin folders
gulp.task('MF-debug-personal-plugins', function ()
{
    //Only run if theres something to compile
    if (Exists(originalDirectory))
    {
